(function() {
  ...
  function FornecedoresCtrl($http, $location, msgs, tabs) {
    const vm = this;
      ...
      $http
        ...
        .then(function(response) {
          $http.get(`${url}/count`).then(function(response) {
            ...
            vm.fornecedor = {
              codigo: String(response.data.value + 1)
              // endereco: [{}],
              // dependentes: [{}]
            };
            vm.endereco = {};
    ...
    vm.getByCep = function() {
      ...
      vm.fornecedor.endereco;
    };

    vm.addEndereco = function() {
      if (!vm.fornecedor.endereco) {
        vm.fornecedor.endereco = [];
      }
      vm.fornecedor.endereco.splice(
        vm.fornecedor.endereco.length,
        0,
        vm.endereco
      );
      vm.endereco = {};
    ...
    vm.deleteEndereco = function(index) {
      if (vm.fornecedor.endereco.length > 1) {
        vm.fornecedor.endereco.splice(index, 1);
      }
    };
    ...
    vm.showTabUpdate = function(fornecedor) {
      vm.fornecedor = fornecedor;
      tabs.show(vm, { tabUpdate: true });
    };

    vm.create = function() {
      vm.fornecedor.tipo = ["provider"];
      $http
        .post(`${url}/create`, vm.fornecedor)
        .then(function(response) {
          vm.refresh();
    ...
    vm.showProdutos = function(fornecedor) {
      vm.fornecedor = fornecedor;
      vm.produtos = {};
      $http.get(`${url}/${fornecedor._id}/produtos`).then(function(response) {
        let produtos = response;
        vm.produtos = produtos.data;
        console.log(vm.produtos);
      });
    };

    vm.update = function() {
      const updateUrl = `${url}/update/${vm.fornecedor._id}`;
      $http
        .put(updateUrl, vm.fornecedor)
        .then(function(response) {
          vm.refresh();
          msgs.addSuccess("Operação realizada com sucesso!");
          vm.showProdutos(vm.fornecedor);
        })
        .catch(function(response) {
          msgs.addError(response.data.errors);
        });
