(function() {
  ...
  function PedidosCtrl($http, $location, msgs, tabs) {
    ...
    vm.changeTeste = function(days) {
      let codigoEtiqueta = vm.codigoEtiqueta;
      let uri = `http://localhost:3003/api`;
      //Consulta todos os produtos que tenha o inicio igual o codigo da etiqueta

      $http
        .get(`${uri}/produto/getProdutos?codigo=${codigoEtiqueta}`)
        .then(response => {
    ...
    vm.refresh = function() {
      ...
      $http
        .get(`${url}/lista?skip=${(page - 1) * 10}&limit=10`)
    ...
    vm.getServicosList = function(peca, i) {
      ...
      $http.get(`${url}?peca=${peca}`).then(function(response) {
        ...
        if (i) {
          vm.pedido.itens[i].valor = 0;
        }
      });
    };

    vm.getProdutosList = function(codigo, i) {
      const url = `http://localhost:3003/api/produto/getProdutos`;
      $http.get(`${url}?codigo=${codigo}`).then(function(response) {
        vm.produtos = Object.keys(response.data).map(function(k) {
          return response.data[k].codigo;
        });
        if (i) {
          vm.pedido.itens[i].valor = 0;
        }
    ...
    vm.getValorServico = function(item, i) {
      ...
      if (item && item.servicos[i].descricao) {
        ...
        $http
          .get(`${url}?peca=${item.peca}&servico=${desc}`)
          .then(function(response) {
    ...
    vm.setEntrega = function(endereco) {
      if (endereco === vm.entrega) {
        ...
        vm.pedido.endEntrega = `${endereco.rua || ""}, ${endereco.numero ||
    ...
    vm.checkStatus = function() {
      ...
      ck == true
        ? (vm.pedido.status = "CONCLUÍDO")
        : (vm.pedido.status = "EM ABERTO");
    };
    ...
    vm.checkServicoStatus = function() {
      ...
      ck == true
        ? (vm.it.statusItem = "CONCLUÍDO")
        : (vm.it.statusItem = "PENDENTE");
    };
    ...
    vm.showTabUpdate = function(pedido) {
      ...
      vm.pedido.financeiro.vencimento = vm.convertDate(
        vm.pedido.financeiro.vencimento
      );
    ...
    //Adiciona Produto
    vm.addItemProduto = function(produto) {
      if (!vm.it) {
        vm.it = {};
      } else {
        vm.it = {
      ...
      console.log(vm.pedido.itens, vm.it);
      vm.calcItens();
    };
    ...
    vm.calcPagamentos = function() {
      ...
      if (vm.pedido.financeiro.creditos.length > 0) {
        vm.pedido.financeiro.creditos.forEach(function(value) {
          vm.vlrPagtos +=
            !value.valor || isNaN(value.valor) ? 0 : parseFloat(value.valor);
        });
      }
    ...
    vm.calcServicos = function() {
      ...
      if (vm.it.servicos.length > 0) {
        vm.it.servicos.forEach(function(v) {
          vm.it.valorTotal +=
            !v.valor || isNaN(v.valor) ? 0 : parseFloat(v.valor);
        });
      }
    };
    ...
    vm.calcItens = function() {
      ...
      if (vm.pedido.itens.length > 0) {
        vm.pedido.itens.forEach(function(value) {
          vm.vlrPedido +=
            !value.valorTotal || isNaN(value.valorTotal)
              ? 0
              : parseFloat(value.valorTotal);
        });
      }
    ...
    vm.calcSubTotal = function() {
      ...
      if (vm.pedido) {
        vm.debito +=
          !vm.pedido.financeiro.debitos.valorPedido ||
          isNaN(vm.pedido.financeiro.debitos.valorPedido)
            ? 0
            : parseFloat(vm.pedido.financeiro.debitos.valorPedido);
        vm.debito +=
          !vm.pedido.financeiro.debitos.frete ||
          isNaN(vm.pedido.financeiro.debitos.frete)
            ? 0
            : parseFloat(vm.pedido.financeiro.debitos.frete);
    ...
    vm.calculateValues = function() {
      if (vm.pedido) {
        vm.pedido.financeiro.valorFinal =
          vm.pedido.financeiro.subTotal - vm.pedido.financeiro.valorPago;
