(function() {
  ...
  function ServicosCtrl($http, $location, msgs, tabs) {
    ...
    vm.refresh = function() {
      ...
      $http.get(`${url}?skip=${(page - 1) * 100}&limit=100`).then(function(response) {
    ...
    vm.create = function() {
      $http.post(url, vm.servico).then(function(response) {
    ...
    vm.update = function() {
      ...
      $http.put(updateUrl, vm.servico).then(function(response) {
        vm.refresh()
        msgs.addSuccess('Operação realizada com sucesso!')
      }).catch(function(response) {
        msgs.addError(response.data.errors)
